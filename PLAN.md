...
- [x] **Google reCAPTCHA v2** («Я не робот») на формі заявки з реальною серверною перевіркою через `siteverify` (2026-07-16). Ключі — в опціях: Налаштування сайту → Заявки; капча активна лише коли заповнені ОБИДВА ключі (без ключів форма працює як раніше: honeypot + rate-limit). Локально перевірено офіційними тестовими ключами Google (вони завжди проходять — на dev так і задумано); відсічення невалідного токена доведено підміною секрета (siteverify → false → 400). На проді потрібні реальні ключі: https://www.google.com/recaptcha/admin/create → тип reCAPTCHA v2 checkbox → домен сайту.
