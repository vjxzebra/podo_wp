...
- [x] **MCP-сервер CRUD записів** (2026-07-27): mu-плагін `podo-mcp.php` — Model Context Protocol (Streamable HTTP, JSON-RPC) на `/wp-json/podo/v1/mcp`; інструменти list/get/create/update/delete_post + list_categories; Basic-автентифікація через `wp_authenticate` (пароль адмінки або Application Password), лише `manage_options`; на проді тільки HTTPS, rate-limit 10 фейлів/15 хв з IP, Origin-перевірка. Пройшов адверсарійне рев'ю (безпека/протокол/WP): виправлено обробку дат (post_date_gmt, таймзона ISO-8601). Підключення — див. DEPLOY.md.
- [x] **+4 нові статті блогу** (2026-07-27, лише прод): «Оніхолізис» (Захворювання нігтів, id 111), «Мозолі та натоптиші» (Догляд за стопами, id 116), «Взуття та шкарпетки при діабеті» (Діабетична стопа, id 121), «Підошовні бородавки у дітей» (Дитяча подологія, id 126) — по ~1500 слів, обкладинки серії, по 2 ілюстрації, перелінкування, seo_description; дати публікацій рознесені 19.06–21.07.
